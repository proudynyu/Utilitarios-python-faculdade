...
void newChar() {
	...
	for (unsigned char i = 0; i < 8; i ++) {
		...
		for (unsigned char k = 0; k < 7; k++) {
			setLCD(pgm_read_byte(&bigNum[k+j]), 1);
		};
...
void writeBig(unsigned char cursor, unsigned char num) {
	setLCD(cursor, 0);
	setLCD(pgm_read_byte(&nr_grande[num][0]), 1);
	setLCD(pgm_read_byte(&nr_grande[num][1]), 1);
	setLCD(cursor + 64, 0);
	setLCD(pgm_read_byte(&nr_grande[num][2]), 1);
	setLCD(pgm_read_byte(&nr_grande[num][3]), 1);
};
